...
bool CEFileFile::ExistsOriginal(const CURL& url)
{
  return CFile::Exists(GetTranslatedPath(url));
}

bool CEFileFile::IsDisc(const CURL& url)
{
  return !IsEFile(url);
}

bool CEFileFile::IsEFile(const CURL& url)
{
  return GetXMLString(url, "efilestub", "path");
}
...
{
  std::string strValue;
  GetXMLString(url, "efilestub", "path", strValue);
#ifdef HAS_DVD_DRIVE
  if (strValue.empty())
    return URIUtils::GetOpticalMediaPath();
#endif
  return strValue;
}

bool CEFileFile::GetXMLString(const CURL& url, std::string strRootElement, std::string strXMLTag, std::string& strValue)
{
  ...
  if (stubXML.LoadFile(strFilename))
  {
    ...
    if (pRootElement && (strRootElement.empty() || StringUtils::CompareNoCase(pRootElement->Value(), strRootElement) == 0))
    {
      if (!strXMLTag.empty()) {
