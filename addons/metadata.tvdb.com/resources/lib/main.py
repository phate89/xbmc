...
def search_series_api(title): #todo add language 
    search= tvdb.Search()
    ret = None 
    try:
        ret = search.series(title)
    except:
        pass
    ADDON.setSetting('token', tvdb.KEYS.API_TOKEN)
    return ret
    
def get_series_details_api(id, all=True): #todo add language 
    show = tvdb.Series(id)
    if all:
        try:
            show.info()
...
def get_series_episodes_api(id):
    ret = None
    showeps = tvdb.Series_Episodes(id)
    try:
        ret = showeps.all()
    except:
        pass
...
def get_episode_details_api(id):
    ep = tvdb.Episode(id)
    try:
        ep.info()
    except:
...
def get_series_details(id):
    ...
    liz.setRating("tvdb", show.siteRating, show.siteRatingCount, True ) #todo check if default rating is tvdb or imdb
    if show.imdbId:
        liz.setUniqueIDs({ 'tvdb': show.id, 'imdb' : show.imdbId }, 'tvdb')
        imdb_info = get_imdb_info(show.imdbId)
        if imdb_info['votes']>0:
            liz.setRating("imdb", imdb_info['rating'], imdb_info['votes'])
    else:
...
def get_series_episodes(id):
    ...
    for ep in episodes:
        ...
        liz.setInfo('video',
                    {'title': ep['episodeName'],  #todo use setting to use dvd season and episode nr
...
def get_episode_details(id):
    ...
    liz=xbmcgui.ListItem(ep.episodeName, offscreen=True)
    liz.setInfo('video',
                {'title': ep.episodeName, #todo use dvd season
                 'episode': ep.airedEpisodeNumber,
                 'season': ep.airedSeason,
                 'plot': ep.overview,
                 'credits': ep.writers,
    ...
    if ep.airsAfterSeason >= 0:
        liz.setInfo('video',
                    {'sortseason': 10000,
                     'sortepisode': ep.airsAfterSeason
                    })
    elif ep.airsBeforeSeason >= 0:
        liz.setInfo('video',
                    {'sortepisode': ep.airsBeforeSeason,
                     'sortseason': ep.airsBeforeEpisode
                    })

    liz.setRating("tvdb", ep.siteRating, ep.siteRatingCount, True ) #todo check if default rating is tvdb or imdb
    if ep.imdbId:
...
def run():
    ...
    if 'action' in params:
        ...
        if action == 'find' and 'title' in params:
            search_series(urllib.unquote_plus(params["title"]), params.get("year", None))
        elif action == 'getdetails' and 'url' in params:
            get_series_details(urllib.unquote_plus(params["url"]))
        elif action == 'getepisodelist' and 'url' in params:
            get_series_episodes(urllib.unquote_plus(params["url"]))
        elif action == 'getepisodedetails' and 'url' in params:
            get_episode_details(urllib.unquote_plus(params["url"]))
        elif action == 'getartwork' and 'id' in params:
            get_artworks(urllib.unquote_plus(params["id"]))
